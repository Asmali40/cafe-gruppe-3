...
from cafe.config.config import resources_path
...
from cafe.config.dev_modus import dev_db_populate
from cafe.repository import engine
...
class DbPopulateService:
    ...
    def _load_csv_files(self) -> None:
        logger.debug("begin")
        tabellen: Final = ["cafe", "produkt", "cafe_manager"]
        csv_path: Final = "/init/cafe/csv"
        # siehe extras/compose/postgres/compose.init.yml
        with self.engine_admin.connect() as connection:
            connection.execute(text("SET search_path TO cafe;"))
