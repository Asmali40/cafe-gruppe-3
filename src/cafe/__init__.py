...
Das Basis-Verzeichnis `cafe` (direkt unter `src`) wird durch `__init__.py` als Modul
für das Projekt deklariert, damit `cafe` als oberster Namensraum bei den Imports
verwendet werden kann.

Außerdem kann mit dem Package Manager _uv_ das Modul als Skript aufgerufen werden:
`uv run cafe`. Siehe `project.scripts` in `pyproject.toml`.
...
uvicorn src.cafe:app --ssl-certfile=src\cafe\config\resources\tls\certificate.crt --ssl-keyfile=src\cafe\config\resources\tls\key.pem
```
...
from cafe.asgi_server import run
from cafe.fastapi_app import app
