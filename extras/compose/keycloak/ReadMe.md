...
Konfigurationsdatei `src\cafe\config\resources\app.toml` in der _Table_ `[keycloak]`
eingetragen:
...
Die Defaultwerte sind in `src\cafe\config\keycloak.py` definiert.

## Client Secret

In der Datei `src\cafe\config\resources\app.toml` muss man `client-secret` auf
...
Diese Zeichenkette wird auch für _Bruno_ und `extras\compose\cafe\app.toml`
benötigt.
