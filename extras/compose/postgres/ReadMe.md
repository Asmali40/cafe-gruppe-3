...
DB `cafe` mit dem DB-User `cafe`anzulegen. Mit dem 2. Skript wird das
Schema `cafe` mit dem DB-User `cafe` als _Owner_ angelegt:
...
- Wie kann ich Cafes aus dem Postleitzahlengebiet "1" selektieren?
- Den Benutzernamen benötige ich nicht.
- Wie kann ich die Daten der 1. Abfrage als CSV exportieren?

**BEACHTE**:

- _Cafes_ als Plural der DB-Tabelle `cafe`.
