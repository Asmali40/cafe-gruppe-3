...
- in `extras\compose\cafe\.env` die Umgebungsvariable `LOGURU_LEVEL=INFO` setzen,
  d.h. das Kommentarzeichen entfernen
- in `extras\compose\cafe\cafe.toml`
  - bei der Table `[cafe.db]` die Property `log-statements` auskommentieren
  - bei der Table `[cafe.graphql]` die Property `graphiql-enabled` auskommentieren
