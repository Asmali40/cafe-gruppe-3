...
"""Fixture für pytest: Repository, Cafe(Write)Service, KeycloakAdmin, UserService."""

from keycloak import KeycloakAdmin
from pytest import fixture
from pytest_mock import MockerFixture

from cafe.repository import CafeRepository
from cafe.security import UserService
from cafe.service import CafeService, CafeWriteService
...
@fixture()
def cafe_repository() -> CafeRepository:
    """Fixture für CafeRepository."""
    return CafeRepository()


@fixture
def cafe_service(cafe_repository: CafeRepository) -> CafeService:
    """Fixture für CafeService."""
    return CafeService(cafe_repository)
...
def keycloak_admin_mock(mocker: MockerFixture) -> KeycloakAdmin:
    ...
    keycloak_admin_cls_mock = mocker.patch(
        "cafe.security.user_service.KeycloakAdmin"
    )
...
def user_service(keycloak_admin_mock) -> UserService:
    ...
    keycloak_admin_mock.get_client_roles.return_value = [cafe_rolle_mock]
    return UserService()


@fixture
def cafe_write_service(
    cafe_repository: CafeRepository, user_service: UserService
) -> CafeWriteService:
    """Fixture für CafeWriteService."""
    return CafeWriteService(cafe_repository, user_service)
